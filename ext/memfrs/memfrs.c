...
int memfrs_enum_proc_list( uint64_t kpcr_ptr, CPUState *cpu )
{
    ...
    printf("Eprocess              CR3                 Full Process Path / [Process Name]\n");
    printf("----------------------------------------------------------------------------\n");
    do {
        ...
        printf( "0x%-20lx%-20lx", eprocess_ptr, cr3 );
        ...
        if( memfrs_get_virmem_content( cpu, cr3, buf_ptr, sizeof(buf), (uint8_t*)buf ) != -1 ){
            for(i=0;i<length;i=i+1)
                printf("%c", (char)(*(buf+i)));
        }
        else {
            printf("[Process Name] %-15s", buf);
        }
