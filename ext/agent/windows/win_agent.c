...
///
/// No return value
static void execute_cmd(char *sCmdline) {
...
///
/// No return value
static void invoke_cmd(char *sCmdline) {
    PROCESS_INFORMATION pi;
    STARTUPINFO si;
    ZeroMemory(&si, sizeof(si));
    ZeroMemory(&pi, sizeof(pi));

    CreateProcess(NULL, sCmdline, NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi);
}
...
///
/// No return value
static void import_cmd(char *filePath, SOCKET sClientSocket)
{
	...
	recv( sClientSocket, (char*)&fileSize, sizeof(fileSize), MSG_WAITALL );
    ...
    if (g_hLog == INVALID_HANDLE_VALUE) {
        display_error("import_cmd - CreateFile - can't open file", TRUE);
		return;
	}
	...
	if( nBytesRead ) {
		...
		SetFilePointer( hFile, fileSizeStored, NULL, FILE_BEGIN );
		SetEndOfFile( hFile );
	}
...
///
/// No return value
static void export_cmd(char *filePath, SOCKET sClientSocket)
{
    ...
    while (sendRound) {
        ...
        nBytesWrite += send(sClientSocket, fBuf, nBytesRead, 0);
...
static void export_log( SOCKET sClientSocket )
{
	...
	// Duplicate the global log handle
	DuplicateHandle( 
		GetCurrentProcess(),
		g_hLog,
		GetCurrentProcess(),
	...
	if (!GetFileSizeEx(hLog, &fileSize)) {
    	display_error("export_log - GetFileSizeEx", TRUE);
    	return;
	}
        ...
        do {
            ...
            if (iResult > 0) {
                ...
                switch (command_code) {
                    ...
                    case MBA_CMD_EXEC :
                        execute_cmd(sCommand_line);
                        break;

                    case MBA_CMD_INVO :
                        invoke_cmd(sCommand_line);
                        break;

                    case MBA_CMD_EXPO :
                        export_cmd(sCommand_line, sClientSocket);
                        break;

                    case MBA_CMD_IMPO :
                        import_cmd(sCommand_line, sClientSocket);
                        break;

                    case MBA_CMD_LOGF :
                        export_log( sClientSocket );
                        break;
