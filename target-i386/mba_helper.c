...
void helper_tracer_dispatcher( CPUX86State* env, uint64_t pc ) {
    ...
    if(tracer_cxt.process_tracer_head!=NULL && !tracer_is_kern_addr(pc)){
    	LL_FOREACH( tracer_cxt.process_tracer_head , cb_rec){
    ...
    if( tracer_cxt.universal_kernel_tracer_head!=NULL && tracer_is_kern_addr(pc) ){
            LL_FOREACH( tracer_cxt.universal_kernel_tracer_head , cb_rec){
            if(cb_rec->enabled == 1)
                cb_rec->cb_func( ENV_GET_CPU(env), pc, 0 );
        }
    }

    if( tracer_cxt.universal_tracer_head!=NULL && !tracer_is_kern_addr(pc) ){
            LL_FOREACH( tracer_cxt.universal_tracer_head , cb_rec){
            if(cb_rec->enabled == 1)
                cb_rec->cb_func( ENV_GET_CPU(env), pc, 0 );
        }
    }    
...
void helper_btracer_dispatcher( CPUX86State* env, uint64_t bstart, uint64_t bend ) {
    ...
    if( tracer_cxt.process_btracer_head!=NULL && !tracer_is_kern_addr(bstart) ){
            LL_FOREACH( tracer_cxt.process_btracer_head , cb_rec){
            if(env->cr[3] == cb_rec->cr3  && cb_rec->enabled == 1)
                cb_rec->cb_func( ENV_GET_CPU(env), bstart, bend );
        }
    }

    if( tracer_cxt.universal_kernel_btracer_head!=NULL && tracer_is_kern_addr(bstart) ){
            LL_FOREACH( tracer_cxt.universal_kernel_btracer_head , cb_rec){
            if(cb_rec->enabled == 1)
                cb_rec->cb_func( ENV_GET_CPU(env), bstart, bend );
        }
    }

    if( tracer_cxt.universal_btracer_head!=NULL && !tracer_is_kern_addr(bstart) ){
            LL_FOREACH( tracer_cxt.universal_btracer_head , cb_rec){
            if(cb_rec->enabled == 1)
                cb_rec->cb_func( ENV_GET_CPU(env), bstart, bend );
        }
    }
