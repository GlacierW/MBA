...
static void gen_dift_sync_i64( DisasContext* s ) {
    ...
    if( !label_or_helper_appeared )
        tcg_gen_op1i( INDEX_op_qemu_dift_inc_diftcodes, s->tb->dift_code_idx );
}
...
static void gen_dift_enqueue_addr( DisasContext* s, int is_wa ) {
    ...
    if( is_wa ) 
        ...
        tcg_gen_op0( &tcg_ctx, INDEX_op_qemu_dift_enq_ra );
}
...
/**********************/
    ...
    /* now check op code, insn. comment in Intel syntax */
 reswitch:
    switch(b) {
    case 0x0f:
        /**************************/
...
#if defined(CONFIG_DIFT)
            // EIP -> [ESP]
            gen_dift_mem_reg( s, R_RIP, EFFECT_ASSIGN | EFFECT_ONE_TO_ONE, ot, 0 );
            // TMP -> EIP
            gen_dift_reg_reg( s, R_RIP, R_DIFT_TMP, EFFECT_ASSIGN | EFFECT_ONE_TO_ONE, ot, 0, 0 );
            // Clear TMP
...
/* Modified by Glacier */
            ...
            // OP -> EIP 
            if( s->src_type == OPT_REG )
                gen_dift_reg_reg( s, R_RIP, s->src_reg, EFFECT_ASSIGN | EFFECT_ONE_TO_ONE, ot, 0, 0 );
            else
                gen_dift_reg_mem( s, R_RIP, EFFECT_ASSIGN | EFFECT_ONE_TO_ONE, ot, 0 );
            gen_dift_sync_i64( s );
#endif
/***********************/
            gen_op_jmp_v(cpu_T[0]);
...
/* Modified by Glacier */
#if defined(CONFIG_DIFT)
        gen_dift_reg_mem( s, R_RIP, EFFECT_ASSIGN | EFFECT_ONE_TO_ONE, s->dflag + MO_16, 0 );
        gen_dift_sync_i64( s );
#endif
/***********************/
...
/* Modified by Glacier */
#if defined(CONFIG_DIFT)
        gen_dift_reg_im( s, R_RIP, dflag + MO_16, 0 );
        gen_dift_sync_i64( s );
#endif
/***********************/
        tval += s->pc - s->cs_base;
